...
require_once("clasedb.php");
 ...
 $r = mysqli_query($db,"SELECT * FROM usuarios WHERE correo='".$correo."' AND clave='$clave' ");
...
for ($i=1; $i <= 18; $i++) { 
 ...
 function listarUsuario(){
  global $db;
  $resultados = [];
  $r = mysqli_query($db,"SELECT  * from usuarios ORDER BY id DESC");
  while($temporal = mysqli_fetch_assoc($r) ) $resultados[] = $temporal;
  return $resultados;
}
 function eliminarUsuario(){ 
  global $db;
  mysqli_query($db,"DELETE FROM usuarios WHERE id=$_REQUEST[id]");
